...
import { db, AppUser } from "@/lib/database";
import { cn } from "@/lib/utils";
import { toast } from "sonner";

export default function AccountSettingsTab({ user, onUpdate }: { user: AppUser; onUpdate: () => void }) {
  ...
  const handleSaveProfile = () => {
    ...
    setSaving(true);
    db.updateUser(user.id, { name: name.trim(), profilePhoto: profilePhoto || undefined, bankAccount: bankAccount || undefined });
    onUpdate();
    setSaving(false);
  ...
  const handleChangePin = () => {
    ...
    db.updateUser(user.id, { password: newPin });
    ...
    <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground">Phone (ใช้เป็น username)</Label>
            <Input value={user.phone} disabled className="bg-muted text-muted-foreground" />
          </div>

          <Button onClick={handleSaveProfile} disabled={saving} className="bg-blue-600 hover:bg-blue-700 gap-2">
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
        <div className="flex items-center gap-3 px-5 py-4 border-b border-border">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">เลขบัญชีสำหรับรับเงิน</p>
          </div>
        </div>
        <div className="p-5 space-y-4">
          <div className="space-y-1.5">
            ...
            />
            <p className="text-xs text-muted-foreground">ระบุชื่อธนาคารและเลขบัญชีเพื่อให้ทีมอ้างอิงได้</p>
          </div>
          <Button onClick={handleSaveProfile} disabled={saving} className="bg-green-600 hover:bg-green-700 gap-2">
            <Save className="w-4 h-4" /> บันทึกบัญชีธนาคาร
          </Button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
        <div className="flex items-center gap-3 px-5 py-4 border-b border-border">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">รหัสผ่านสำหรับเข้าสู่ระบบ</p>
          </div>
        </div>
        <div className="p-5 space-y-4">
          <div className="space-y-1.5">
            <Label>รหัสผ่านปัจจุบัน</Label>
            <Input type="password" placeholder="••••" value={currentPin} onChange={(e) => setCurrentPin(e.target.value)} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label>รหัสผ่านใหม่</Label>
              <Input type="password" placeholder="••••" value={newPin} onChange={(e) => setNewPin(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label>ยืนยันรหัสผ่านใหม่</Label>
              <Input type="password" placeholder="••••" value={confirmPin} onChange={(e) => setConfirmPin(e.target.value)} />
            </div>
          </div>
          <Button onClick={handleChangePin} variant="outline" className="gap-2 border-amber-300 text-amber-700 hover:bg-amber-50">
